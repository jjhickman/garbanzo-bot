...
// @ts-nocheck -- mechanical byte-compatible port; typed config logic lives in src/config-core.
...
import {
  writeFileWithBackupAtomic,
  writeJsonWithBackupAtomic,
} from '../../config-core/writers.js';
...
async function main() {
  ...
  try {
    ...
    const finalEnv = {
      ...
      WHATSAPP_LOGIN_TOKEN: (whatsappEnv.WHATSAPP_LOGIN_TOKEN || existing.WHATSAPP_LOGIN_TOKEN || '').trim(),
      WHATSAPP_CHAT_SCOPE: (whatsappEnv.WHATSAPP_CHAT_SCOPE || existing.WHATSAPP_CHAT_SCOPE || 'all').trim(),
      WHATSAPP_SET_PROFILE_NAME: (whatsappEnv.WHATSAPP_SET_PROFILE_NAME || existing.WHATSAPP_SET_PROFILE_NAME || 'true').trim(),
    ...
    if (dryRun) {
      ...
      for (const target of envTargets) {
        ...
        writeFileWithBackupAtomic(target.path, target.content);
    ...
    if (shouldWriteGroups) {
      ...
      if (dryRun) {
        ...
        writeJsonWithBackupAtomic(GROUPS_PATH, groupsConfig);
    ...
    if (messagingPlatform === 'discord') {
      if (!discordChannelsChanged) {
        ...
        if (dryRun) {
          ...
          writeJsonWithBackupAtomic(DISCORD_CHANNELS_PATH, channelsConfig);
    ...
    if (messagingPlatform === 'telegram') {
      if (!telegramChatsChanged) {
        ...
        if (dryRun) {
          ...
          writeJsonWithBackupAtomic(TELEGRAM_CHATS_PATH, telegramChatsConfig);
    ...
    if (messagingPlatform === 'matrix') {
      if (!matrixRoomsChanged) {
        ...
        if (dryRun) {
          ...
          writeJsonWithBackupAtomic(MATRIX_ROOMS_PATH, matrixRoomsConfig);
