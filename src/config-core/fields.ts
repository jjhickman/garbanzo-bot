...
import { isSecretKey } from './secret-classifier.js';

export { redactEnvContent } from './secret-classifier.js';
...
type RawSetupField = Omit<SetupField, 'secret'> & { secret?: boolean };
type EnvValues = Readonly<Record<string, string | undefined>>;
type CliValues = { options: Readonly<Record<string, string | undefined>> };
 ...
 */
export const SHARED_FIELDS: readonly RawSetupField[] = [
...
export const WHATSAPP_FIELDS: readonly RawSetupField[] = [
...
export const DISCORD_FIELDS: readonly RawSetupField[] = [
...
export const TELEGRAM_FIELDS: readonly RawSetupField[] = [
...
export const MATRIX_FIELDS: readonly RawSetupField[] = [
...
export const FIELD_TABLE: readonly SetupField[] = [
  ...SHARED_FIELDS,
  ...WHATSAPP_FIELDS,
  ...DISCORD_FIELDS,
  ...TELEGRAM_FIELDS,
  ...MATRIX_FIELDS,
].map((field) => ({ ...field, secret: isSecretKey(field.env) }));
...
function envLine(values: EnvValues, key: string): string {
  return `${key}=${values[key] ?? ''}`;
}

function envKeysFromLines(lines: readonly string[]): string[] {
  return lines
...
export function buildSharedEnvLines(values: EnvValues): string[] {
  return [
...
export function buildPlatformEnvLines(platform: string, values: EnvValues): string[] {
  if (platform === 'discord') {
    return [
...
export function getField(env: string): SetupField {
 ...
 */
export function resolveEnvField(field: SetupField, cli: CliValues, existing: EnvValues): string {
 ...
 */
export function promptHint(field: SetupField, existing: EnvValues): string {
  if (field.secret) {
...
export function mergeExistingEnvForPlatform(rootEnv: EnvValues, platformEnv: EnvValues = {}): Record<string, string | undefined> {
  return { ...rootEnv, ...platformEnv };
}

export function mergeEnvFileContent(existingContent: string, generatedContent: string): string {
  ...
  const missingGeneratedLines: string[] = [];
...
export function promptFieldEnvsForPlatform(platform: string): string[] {
...
export function emittedKeysForPlatform(platform: string): { sharedKeys: string[]; platformKeys: string[] } {
  return {
 ...
 */
export function resolveComposeProfiles(platform: string, monitoringEnabled: boolean): string {
 ...
 * operator twice.
 */
export function resolveMessagingPlatform(cli: CliValues, existing: EnvValues): string {
