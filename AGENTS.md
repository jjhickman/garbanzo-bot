...
- **Bridge groups (v3.3.0):** a bridge route is N-ary (`endpoints` has 2+ members). A source message fans out to every other endpoint in the group, with a target-scoped idempotency key (JSON-array encoding of `origin instance/chat/message + target instance/chat` — injective even when ids contain `:`, as Matrix room/event ids do, so receiver `bridge_seen` dedup never collapses sibling fan-out legs nor drops a real leg via a colliding key). Relays remain non-transitive because delivery is a direct send and is never re-captured. A route/group lists **at most one endpoint per instance** — a bridge member is one distinct instance (two WhatsApp numbers = two `INSTANCE_ID`s, not one instance with two chats); run a second instance if you need two chats on the same platform. This keeps summary buffering and one-way `from` authority unambiguous (each resolves an instance to exactly one endpoint).
