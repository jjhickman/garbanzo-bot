...
- **Bridge groups (v3.3.0):** a bridge route is N-ary (`endpoints` has 2+ members). A source message fans out to every other endpoint in the group, with a target-scoped idempotency key (`origin instance/chat/message + target instance/chat`) so receiver dedup never collapses sibling fan-out legs. Relays remain non-transitive because delivery is a direct send and is never re-captured. Groups should normally use distinct instances; same-instance different-chat endpoints are schema-valid but transport/runtime loopback is not the recommended deployment shape.
