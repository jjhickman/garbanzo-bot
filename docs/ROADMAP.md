...
6. ~~**Character creation** (`src/features/character.ts`) — D&D 5e character sheet PDF generation via `pdf-lib`, official WotC fillable template, stat calculation, Baileys document upload~~ ✅ Live — `!character`, `!char [race] [class]`, 4d6-drop-lowest stats, class-priority assignment, racial bonuses, all 3 PDF pages filled (page 1: stats/combat/race traits, page 2: appearance/backstory/class features/treasure, page 3: spellcasting for caster classes), natural language routing ("make me an elf wizard")
