...
class GoogleWeather:
    ...
    def format_summary(self, data, lang="en"):
        ...
        # Convert to Fahrenheit and mph for US
        temp_f = self._c_to_f(temp.get('current'))
        feels_f = self._c_to_f(temp.get('feels_like'))
        wind_mph = self._kmh_to_mph(wind.get('speed'))
        ...
        if lang == "he":
            ...
            lines = [
                f"*{data.get('location', 'Unknown')}*",
                f"{desc} {emoji}".strip(),
                f"🌡️ {temp_f}°F (feels like {feels_f}°F)",
                f"💨 Wind: {wind_mph} mph {wind.get('direction', '')}",
    ...
    def format_forecast(self, data, lang="en"):
        ...
        for i, h in enumerate(data['hourly']):
            if i % 4 == 0:
                ...
                temp_f = self._c_to_f(h['temp'])
                wind_mph = self._kmh_to_mph(h['wind']['speed'])
                
                if lang == "he":
                    lines.append(f"{time_str}: {h['temp']}°C, {h['condition']['emoji']} {h['wind']['speed']} קמ\"ש {h['wind']['direction']}")
                else:
                    lines.append(f"{time_str}: {temp_f}°F, {h['condition']['emoji']} {wind_mph} mph {h['wind']['direction']}")
